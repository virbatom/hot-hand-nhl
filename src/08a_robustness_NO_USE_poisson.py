# 08a_robustness_NO_USE_poisson.py

"""
Robustness checks for the Hot Hand analysis - POISSON ONLY.
Exposure: Game-clock.
...
All use Poisson + Season FE (Model 2 spec equivalent) as the benchmark.
"""
...
# 2. Helper: run Poisson + season FE on a given sample
def run_poisson(data, season_dum, extra_cols=None, label=""):
    """Run Poisson + season FE. Returns (irr, pval, n, model) or None on failure."""
    t0 = time.time()
    ...
    try:
        # POUŽITIE sm.Poisson NAMIESTO sm.NegativeBinomial
        ...
        print(f"    LL={model.llf:,.1f}  AIC={model.aic:,.1f}  ({time.time()-t0:.1f}s)")
...
print("\n" + "=" * 72)
print("ROBUSTNESS CHECK R0: BASELINE (Poisson + Season FE)")
print("=" * 72)
r = run_poisson(panel, season_dummies, label="Full sample, Poisson + Season FE")
...
r = run_poisson(fwd, s_dum_fwd, label="Forwards only")
...
r = run_poisson(dmen, s_dum_d, label="Defensemen only")
...
r = run_poisson(panel_placebo, s_dum_pl, label="Placebo: first non-goal SOG as treatment")
...
r = run_poisson(early, s_dum_e, label="Scorers: goal within first 40 min only")
...
r = run_poisson(single, s_dum_s, label="Single-goal games only (0 or 1 goal)")
...
r = run_poisson(consec, s_dum_c, label="Consecutive play only (no missed games)")
if r: results["R8. Consecutive play"] = r[:3]

# 4. Summary table
print("\n" + "=" * 72)
print("ROBUSTNESS SUMMARY (POISSON ONLY)")
print("=" * 72)
