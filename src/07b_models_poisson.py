# 07b_models_poisson.py

"""
Rerun key models with shift-based TOI as exposure term - POISSON ONLY.
Uses log(toi_5v5_post) instead of log(time_after_split).

Models:
  M1: Pooled Poisson (TOI exposure)
  M2: Poisson + Season FE (TOI exposure)
  M3: Poisson + Season FE + Score State (TOI exposure)  [KEY MODEL]
  M4: Linear FE with TOI-based rate as dep var
"""
...
# M2: Poisson + Season FE (TOI exposure)
print("\n" + "=" * 72)
print("M2: POISSON + SEASON FE (TOI exposure)")
print("=" * 72)
t0 = time.time()
m2 = sm.Poisson(y, X_season, offset=exposure_toi).fit(disp=0, maxiter=200)
print(f"  Converged in {time.time()-t0:.1f}s")
print_results(m2, season_cols, SHOW)
results["M2. Poisson + Season FE (TOI)"] = (m2, season_cols)

# M3: Poisson + Season FE + Score State (TOI exposure) — KEY MODEL
print("\n" + "=" * 72)
print("M3: POISSON + SEASON FE + SCORE STATE (TOI exposure) *** KEY MODEL ***")
print("=" * 72)
t0 = time.time()
m3ss = sm.Poisson(y, X_ss, offset=exposure_toi).fit(disp=0, maxiter=200)
print(f"  Converged in {time.time()-t0:.1f}s")
print_results(m3ss, ss_cols, SHOW + ["is_leading", "is_trailing"])
results["M3. Poisson + SS (TOI)"] = (m3ss, ss_cols)

# M4: Linear FE with TOI-based rate
print("\n" + "=" * 72)
print("M4: LINEAR OLS + PLAYER FE (TOI-based rate, within-transform)")
print("=" * 72)
...
t0 = time.time()
m4 = sm.OLS(y_fe, X_fe).fit(cov_type="cluster", cov_kwds={"groups": panel_fe["PlayerID"].values})
print(f"  Converged in {time.time()-t0:.1f}s")
print(f"  R-squared (within): {m4.rsquared:.4f}")
print(f"  N: {int(m4.nobs):,}, Clusters: {panel_fe['PlayerID'].nunique():,}")
...
for col in fe_show:
    i = fe_cols.index(col)
    coef = m4.params.iloc[i]
    se = m4.bse.iloc[i]
    pv = m4.pvalues.iloc[i]
    sig = "***" if pv < 0.001 else "** " if pv < 0.01 else "* " if pv < 0.05 else "    "
    print(f"  {col:<32s} {coef:>8.4f} {se:>8.4f} {pv:>8.4f} {sig}")

results["M4. Linear FE (TOI rate)"] = (m4, fe_cols)

# SUMMARY
print("\n" + "=" * 72)
print("MODEL COMPARISON: TOI EXPOSURE (POISSON ONLY)")
print("=" * 72)
