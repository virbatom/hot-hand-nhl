# 08c_robustness_v2_robustness_toi.py

"""
Robustness checks with TOI-based exposure.
All models: NB2 + Season FE + Score State, offset = log(toi_5v5_post).
"""
...
print("=" * 72)
print("ALL MODELS: NB2 + SEASON FE + SCORE STATE, TOI EXPOSURE")
print("=" * 72)
...
r = run_nb(panel_pl, season_dummies, label="Random treatment (seed=296)")
...
panel_pl["scored_first_goal"] = (
    ...
    .transform(lambda x: np.random.permutation(x.values)))
r = run_nb(panel_pl, season_dummies, label="Random treatment (seed=185)")
...
panel_pl["scored_first_goal"] = (
    panel.groupby("Season")["scored_first_goal"]
    .transform(lambda x: np.random.permutation(x.values)))
r = run_nb(panel_pl, season_dummies, label="Random treatment (seed=142)")
...
# Summary
print("\n" + "=" * 72)
print("ROBUSTNESS SUMMARY (TOI exposure, all with score-state controls)")
print("=" * 72)
