...
"""
Hot-Hand Regression Models - POISSON ONLY
Exposure: Game-clock (time_after_split)
"""
...
# Matrix pre Poisson + Season FE + Score State
...
# MODEL 2: Poisson + Season FE
print("\n" + "=" * 72)
print("MODEL 2: POISSON + SEASON FE")
print("=" * 72)
t0 = time.time()
m2 = sm.Poisson(y, X_season, offset=exposure).fit(disp=0, maxiter=200)
print(f"  Converged in {time.time()-t0:.1f}s")
print_results(m2, season_col_list, "Poisson + Season FE", SHOW_COLS)
results["2. Poisson + Season FE"] = (m2, season_col_list)

# MODEL 3: Poisson + Season FE + Score State
print("\n" + "=" * 72)
print("MODEL 3: POISSON + SEASON FE + SCORE STATE *** KEY MODEL ***")
print("=" * 72)
t0 = time.time()
m3 = sm.Poisson(y, X_ss, offset=exposure).fit(disp=0, maxiter=200)
print(f"  Converged in {time.time()-t0:.1f}s")
print_results(m3, ss_col_list, "Poisson + SS", SHOW_COLS + ["is_leading", "is_trailing"])
results["3. Poisson + SS"] = (m3, ss_col_list)

# MODEL 4: Linear FE (within-transformation) as robustness
print("\n" + "=" * 72)
print("MODEL 4: LINEAR OLS WITH PLAYER FIXED EFFECTS (within-transform)")
print("=" * 72)
...
results["4. Linear FE (within)"] = (m4, fe_col_list)
